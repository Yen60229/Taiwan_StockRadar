...
- [x] **修 health check 斷鏈**——**已完成**:後端有 `/api/health`(`/healthz` 在 Caddy 後面會落到 nginx 的 SPA fallback 回 200,不能拿來監控,程式碼裡有註解說明);`deploy.sh` 與兩份部署文件同步改掉。原文:(~半天):後端加 `GET /api/health`(或 Caddyfile 把 `/healthz` route 到 api),同步修 `scripts/deploy.sh:109`、`docs/deploy-to-vps.md:140`、`docs/部署到VPS.md:198`;compose 給 api/frontend/caddy 加 healthcheck
- [x] **拿掉 prod bind mount**——**已完成**:`docker-compose.prod.yml` 只剩 `./logs:/app/logs`,原始碼 bind mount 已移除。原文:(~半天):刪 `docker-compose.prod.yml:47-49,63-65` 的 `./backend:/app`——prod 必須跑 image 而不是 host 原始碼;dev 用 `docker-compose.override.yml` 保留 bind mount
- [x] **鎖住 dev compose**——**已完成**:`5432`/`8000` 都綁 `127.0.0.1`,`SECRET_KEY` 的樣板 fallback 移除且啟動時直接拒絕。原文:(~半天):`5432`/`8000` 改綁 `127.0.0.1:`,刪掉 `SECRET_KEY:-change-me-in-production` fallback(app 啟動時若無 SECRET_KEY 直接 crash);注意 **Docker 的 iptables 會繞過 UFW**,這是文件沒講的坑
- [x] **修 scheduler cron 環境變數 bug**——**已完成**:`cron_entry.sh` 啟動時把需要的變數寫進 `/etc/stockradar.env`(600),各 `run_*.sh` 開頭 source 它。原文:(~1 天):`cron_entry.sh` 的 cron job 拿不到 `DATABASE_URL`(Debian cron 不繼承 daemon env),最小修法是 entrypoint 先 `printenv | grep -E 'DATABASE_URL|RESEND|SECRET' > /etc/environment`;正解是 Phase 2 換掉 cron-in-container
- [x] **上線**——**2026-09 完成,但平台是 Oracle Ampere A1(ARM)不是 Hetzner**:Caddy 已拿到 Let's Encrypt 憑證、`backfill_history.py` 已跑、`oracle-cloud-automation.md` 的個資已清。原文:(~1 週):Hetzner CX22 + UFW + non-root user 照 `docs/deploy-to-vps.md` 走一遍,DNS → Caddy 拿到 Let's Encrypt 憑證,跑 `backfill_history.py`,清掉 `docs/oracle-cloud-automation.md` 裡的真實 Gmail 與 IP(已完成,見 `05-review-2026-09.md`)
- [x] **最低限度備份**——**已完成**:host crontab 每日 03:30 跑 `scripts/backup_db.sh`,並以 `scripts/restore_drill.sh` 實際演練過還原(RTO 3 秒)。異地備份(R2)仍待做。原文:(~2 天):先不求完美——host crontab 每日 `pg_dump | gzip` 到 `/opt/backups`,保留 14 天,**手動做一次 restore 到本地驗證 dump 可用**
